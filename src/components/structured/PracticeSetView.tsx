      ...
      <div className="min-h-0 flex-1 space-y-5 overflow-y-auto p-4 sm:p-5">
        <label className="block">
          ...
          <input
            ...
            className="w-full max-w-xs rounded-xl border border-slate-700/60 bg-slate-900/60 px-3.5 py-2.5 text-sm text-slate-100 outline-none transition-colors focus:border-[var(--mode-accent)]/70"
          />
        </label>

        <div>
          <div className="mb-2 flex items-center justify-between">
            <span className="text-xs font-medium text-slate-400">문제 목록</span>
            <button
              type="button"
              onClick={addProblem}
          ...
          {data.problems.length === 0 ? (
            ...
            <div className="space-y-3">
              {data.problems.map((p, i) => (
                <div key={i} className="rounded-xl border border-slate-700/50 bg-slate-900/50 p-3.5">
                  <div className="flex items-start gap-2">
                    ...
                    <textarea
                      ...
                      rows={2}
                      placeholder="문제 내용"
                      className="w-full flex-1 resize-y rounded-lg border border-slate-700/60 bg-slate-950/60 px-3 py-2 text-sm text-slate-100 outline-none transition-colors focus:border-[var(--mode-accent)]/70"
                    />
                    <button
                      type="button"
                      onClick={() => removeProblem(i)}
                      aria-label="문제 삭제"
                      className="mt-1.5 shrink-0 rounded-lg p-1.5 text-slate-500 transition-colors hover:text-red-400"
                    >
                  ...
                  <div className="mt-2 space-y-1.5 pl-6">
                    {p.choices.map((c, ci) => (
                      <div key={ci} className="flex items-center gap-2">
                        ...
                        <input
                          ...
                          placeholder="보기"
                          className="w-full rounded-lg border border-slate-700/60 bg-slate-950/40 px-3 py-1.5 text-sm text-slate-200 outline-none transition-colors focus:border-[var(--mode-accent)]/70"
                        />
                        <button
                          type="button"
                          onClick={() => removeChoice(i, ci)}
                          aria-label="보기 삭제"
                          className="shrink-0 rounded-md p-1 text-slate-500 transition-colors hover:text-red-400"
                        >
                    ...
                    <button
                      type="button"
                      onClick={() => addChoice(i)}
                      className="flex items-center gap-1 rounded-lg px-2 py-1 text-xs font-medium text-[var(--mode-accent)] transition-colors hover:bg-slate-700/40"
                    >

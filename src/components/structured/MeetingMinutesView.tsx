...
export default function MeetingMinutesView({
  ...
  return (
    <PanelShell
      icon={<ClipboardList className="h-4 w-4 text-[var(--mode-accent)]" />}
      title={t("structured.meetingMinutes.title")}
          ...
          {data.actionItems.length === 0 ? (
            ...
            <div className="space-y-2">
              {data.actionItems.map((item, i) => (
                <div
                  key={i}
                  className="grid gap-2 rounded-xl border border-slate-200 dark:border-slate-800 bg-slate-50 dark:bg-slate-900/50 p-3 sm:grid-cols-[1fr_9rem_9rem_auto] sm:items-center"
                >
                  <input
                    type="text"
                    value={item.task}
                  ...
                  <input
                    ...
                    className="w-full rounded-lg border border-slate-300 dark:border-slate-700 bg-slate-50 dark:bg-slate-950/60 px-3 py-2 text-sm text-slate-900 dark:text-slate-100 outline-none transition-colors focus:border-[var(--mode-accent)]/70 [color-scheme:dark]"
                  />
                  <button
                    type="button"

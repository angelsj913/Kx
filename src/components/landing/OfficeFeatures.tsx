    ...
    <section className="py-20">
      <div className="mx-auto max-w-6xl px-6">
        <div className="text-center">
          ...
          <p className="mx-auto mt-3 max-w-xl text-sm text-slate-600 sm:text-base">{t("office.subtitle")}</p>
        </div>
        ...
        <div className="mt-12 grid gap-6 sm:grid-cols-3">
          {items.map(({ icon: Icon, title, desc, detail }) => (
            <div
              key={title}
              className="group rounded-2xl border border-slate-200 bg-white p-6 shadow-sm transition-all duration-400 ease-[cubic-bezier(0.4,0,0.2,1)] hover:scale-[1.03] hover:border-blue-500/60 hover:shadow-xl hover:shadow-blue-600/10"
            >
              ...
              <div className="grid grid-rows-[0fr] transition-all duration-400 ease-[cubic-bezier(0.4,0,0.2,1)] group-hover:grid-rows-[1fr]">
                <div className="overflow-hidden">
                  <p className="mt-3 text-sm leading-relaxed text-blue-600">{detail}</p>
                </div>
              </div>
            </div>
          ))}

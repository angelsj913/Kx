...
const SCENE_COUNT = 3;
/** 3장면 sticky — 이전 360vh보다 짧게 */
const SKILLS_TRACK_VH = 280;
/** CTA 밴드 — 한 화면 고정 + 짧은 스크롤 런웨이 */
const BAND_TRACK_VH = 180;
      ...
      <section id="skills" className="scroll-mt-24 py-14 sm:py-16">
        <div className="mx-auto max-w-5xl px-6">
          ...
          <div className="mt-12 space-y-12">
            {scenes.map((scene) => (
              <div key={scene.id} className="relative overflow-hidden rounded-2xl py-12 sm:py-14">
...
export default function SkillsSection() {
  ...
  const { sectionRef: bandRef, p: bandP } = useScrollProgress<HTMLElement>({ topOffset: 72 });
  ...
  return (
    <>
      {/* Throttle 1 — 핵심 스킬 (3장면 sticky) */}
      <section ref={sectionRef} id="skills" className="relative scroll-mt-24" style={{ height: `${SKILLS_TRACK_VH}vh` }}>
        <div className="sticky top-0 min-h-[100svh] overflow-hidden">
          ...
          <div className="relative z-10 flex min-h-[100svh] flex-col justify-center py-14 sm:py-16">
            <div className="mx-auto w-full max-w-5xl px-6">
              ...
              <div className="grid items-end gap-10 lg:grid-cols-[1fr_auto]">
                ...
                <ol className="flex list-none items-center gap-3 lg:flex-col lg:items-end lg:gap-4" aria-hidden="true">
                  {scenes.map((scene, i) => (
                    <li key={scene.id} className="flex items-center gap-2 lg:flex-row-reverse">
                      <span
                        className={`block h-1.5 rounded-full transition-all duration-300 ${
                          i === activeIdx ? "w-8 bg-[#2563EB] dark:bg-blue-400" : "w-4 bg-slate-300 dark:bg-slate-600"
                        }`}
                      />
                      <span

      ...
      {open && (
        ...
        >
          <motion.div
            ...
            className="flex h-[32rem] w-full max-w-2xl overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-2xl shadow-slate-900/20 dark:border-slate-700/60 dark:bg-slate-900/95 dark:shadow-black/50 dark:backdrop-blur-md"
          >
            <nav className="flex w-40 shrink-0 flex-col gap-1 border-r border-slate-200 p-3 dark:border-slate-800/60">
              <p className="mb-2 px-2 text-sm font-semibold text-slate-900 dark:text-slate-100">{t("settings.title")}</p>
            ...
            <div className="relative min-w-0 flex-1 overflow-y-auto p-5">
              <button
                type="button"
                onClick={onClose}
                className="absolute right-4 top-4 rounded-lg p-1 text-slate-400 transition-colors hover:bg-slate-100 hover:text-slate-700 dark:hover:bg-slate-800 dark:hover:text-slate-100"
              >
                <X className="h-4 w-4" />
              </button>
      ...
      <div className="space-y-1.5">
        {tools.map((tool) => {
          ...
          return (
            <div
              key={tool.id}
              className="flex items-center justify-between rounded-xl border border-slate-200 bg-slate-50 px-3.5 py-2.5 dark:border-slate-800/60 dark:bg-slate-800/30"
            >
            ...
            <button
              ...
              className={`flex w-full items-center justify-between rounded-xl border px-4 py-3 text-left text-sm font-medium transition-colors duration-200 ${
                active
                  ...
                  : "border-slate-200 bg-slate-50 text-slate-600 hover:border-slate-300 dark:border-slate-800/60 dark:bg-slate-800/30 dark:text-slate-300 dark:hover:border-slate-700"
              }`}
            >
              {t(p.labelKey)}

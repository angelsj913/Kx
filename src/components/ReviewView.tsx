...
export default function ReviewView() {
  ...
  return (
    <div className="mx-auto flex h-full max-w-3xl flex-col overflow-y-auto px-4 py-8 sm:px-6">
      <div className="flex flex-col gap-1.5 sm:flex-row sm:items-center sm:justify-between">
        <div>
          ...
          <p className="mt-1 text-sm text-slate-400">
            오늘 복습할 카드 <span className="font-semibold text-violet-300">{stats.due}</span>개 ·
            전체 {stats.total}개
          </p>
        </div>
        <div className="flex gap-2">
          <button
            ...
            className="flex items-center gap-1.5 rounded-xl border border-slate-700/60 bg-slate-800/50 px-3 py-2 text-sm font-medium text-slate-200 hover:border-violet-500/50"
          >
          ...
          <button
            ...
            className="flex items-center gap-1.5 rounded-xl bg-gradient-to-r from-violet-600 to-indigo-600 px-3 py-2 text-sm font-semibold text-white shadow-lg shadow-violet-900/30"
          >
      ...
      {error && (
        <p className="mt-4 rounded-xl border border-red-500/30 bg-red-950/40 px-4 py-2.5 text-sm text-red-300">
          {error}
        </p>
      )}
          ...
          >
            <div className="space-y-2 rounded-2xl border border-slate-700/60 bg-slate-800/40 p-4">
              ...
              <textarea
                ...
                placeholder="뒷면 (답)"
                rows={2}
                className="w-full resize-none rounded-lg border border-slate-700 bg-slate-900/60 px-3 py-2 text-sm text-slate-100 outline-none focus:border-violet-500"
              />
              <button
                type="button"
                onClick={addCard}
              ...
              {libraryItems.length === 0 ? (
                ...
                <>
                  <select
                    ...
                    className="w-full rounded-lg border border-slate-700 bg-slate-900/60 px-3 py-2 text-sm text-slate-100 outline-none"
                  >
                    {libraryItems.map((it) => (
                  ...
                  <div className="flex items-center gap-2">
                    <label className="text-xs text-slate-400">개수</label>
                    <input
                      type="number"
                      min={3}
                      max={30}
                      ...
                      className="w-20 rounded-lg border border-slate-700 bg-slate-900/60 px-2 py-1.5 text-sm text-slate-100 outline-none"
                    />
                    <button
                      type="button"
                      onClick={generate}
                      disabled={generating}
                      className="ml-auto flex items-center gap-1.5 rounded-lg bg-violet-600 px-4 py-2 text-sm font-semibold text-white disabled:opacity-50"
                    >
          ...
          <div className="w-full">
            ...
            <div
              ...
              className={`flex min-h-[14rem] cursor-pointer flex-col items-center justify-center rounded-3xl border p-8 text-center transition-colors ${
                flipped
                  ? "border-violet-500/40 bg-slate-800/60"
                  : "border-slate-700/60 bg-slate-800/40 hover:border-violet-500/40"
              }`}
            >
              <p className="text-lg font-semibold text-slate-50 whitespace-pre-wrap">{current.front}</p>
              {flipped && (
                <>
                  <div className="my-4 h-px w-16 bg-slate-700" />
                  <p className="text-base text-slate-300 whitespace-pre-wrap">{current.back}</p>
                </>
              )}
              {!flipped && <p className="mt-4 text-xs text-slate-600">눌러서 정답 확인</p>}
            </div>
            ...
            {flipped ? (
              ...
              <div className="mt-4 flex justify-center gap-2">
                <button
                  ...
                  className="flex items-center gap-1.5 rounded-xl bg-violet-600 px-5 py-2.5 text-sm font-semibold text-white"
                >
                ...
                <button
                  ...
                  title="이 카드 삭제"
                  className="rounded-xl border border-slate-700/60 px-3 py-2.5 text-slate-500 hover:text-red-400"
                >

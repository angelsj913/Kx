  ...
  return (
    <div className="mx-auto flex h-full max-w-3xl flex-col overflow-y-auto px-4 py-8 sm:px-6">
      ...
      <form onSubmit={search} className="mt-5 flex items-center gap-2">
        <div className="flex min-w-0 flex-1 items-center gap-2 rounded-xl border border-slate-700/60 bg-slate-900/60 px-3">
          <Search className="h-4 w-4 shrink-0 text-slate-500" />
          <input
            value={query}
            onChange={(e) => setQuery(e.target.value)}
            placeholder="문서에 대해 무엇이든 물어보세요"
            className="min-w-0 flex-1 bg-transparent py-2.5 text-sm text-slate-100 outline-none placeholder:text-slate-600"
          />
        </div>
        <button
          type="submit"
          disabled={searching || !query.trim()}
          className="flex shrink-0 items-center gap-1.5 rounded-xl bg-gradient-to-r from-violet-600 to-indigo-600 px-4 py-2.5 text-sm font-semibold text-white shadow-lg shadow-violet-900/30 disabled:opacity-50"
        >
      ...
      {answer && (
        <motion.div
          ...
          className="mt-5 rounded-2xl border border-violet-500/30 bg-slate-800/50 p-5"
        >
          ...
          {sources.length > 0 && (
            <div className="mt-4 space-y-2 border-t border-slate-700/60 pt-3">
              ...
              {sources.map((s) => (
                <div key={s.n} className="rounded-lg bg-slate-900/50 px-3 py-2 text-xs">
                  <p className="mb-0.5 font-medium text-violet-300">
                    [{s.n}] {s.title} <span className="text-slate-600">· {s.score}</span>
                  </p>
                  <p className="line-clamp-2 text-slate-400">{s.snippet}</p>
                </div>
              ))}
            </div>
          )}
      ...
      <div className="mt-8">
        ...
        {items.length === 0 ? (
          <p className="rounded-xl border border-slate-700/50 bg-slate-800/30 px-4 py-6 text-center text-sm text-slate-500">
          ...
          <ul className="space-y-1.5">
            {items.map((it) => (
              <li
                key={it.id}
                className="flex items-center gap-2.5 rounded-xl border border-slate-700/50 bg-slate-800/30 px-3 py-2.5"
              >
                ...
                <button
                  ...
                  className="flex shrink-0 items-center gap-1.5 rounded-lg border border-slate-700/60 bg-slate-900/60 px-3 py-1.5 text-xs font-medium text-slate-200 hover:border-violet-500/50 disabled:opacity-50"
                >
                  {indexing[it.id] ? (

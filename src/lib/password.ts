 ...
 *
 * 최소 길이만 요구하던 예전 규칙(8자 이상, 그 외 제한 없음)은 "aaaaaaaa" 같은
 * 값도 그대로 통과시켰다. 완전한 무작위 요구는 오히려 사람들이 예측 가능한
 * 패턴(Password1!)으로 우회하게 만들 뿐이라, 길이를 늘리고 문자 종류 4개 중
 * 3개 이상을 요구하는 절충안(NIST SP 800-63B가 권장하는 방향과도 맞음)으로
 * 정했다. 여기에 흔한 비밀번호 차단·개인정보 포함 차단을 더한다.
 */

...
[입력 처리]
1. YouTube 등 URL이 있으면 영상 내용 기준으로 요약 (접근 불가면 URL·제목만으로 구조 잡고, 한계를 명시).
2. 오디오가 있으면 들은 내용을 우선.
3. 이미지가 있으면 슬라이드/판서로 보완.
4. 대본/메모 텍스트가 있으면 그걸 1순위로 정확 요약.
...
- 5~10개
## 타임라인 / 섹션별 정리
- \`00:00\` 형태가 가능하면 사용, 모르면 섹션 제목으로
- 각 섹션 3~6 bullet (사실 위주)
## 상세 노트
- 정의·예시·숫자·고유명사 보존
## 액션 아이템
- 할 일 3~5개
## 복습 질문
- 이해 확인 질문 5개 (답은 짧게 힌트만)

추측 금지. 불명확하면 [불명확] 표시. 한국어.`;

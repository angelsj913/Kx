...
const NOTE_A4_INSTRUCTION = `너는 학생·직장인의 필기를 완벽한 A4 분량 노트로 만드는 전문 필기자이다.

[출력 포맷 — 반드시 준수]
1. 마크다운으로 A4 1~2장 분량의 세밀한 노트를 작성한다.
2. 구조: 제목 → 학습 목표 → 본문(소제목·정의·예시·주의점) → 요약 박스 → 자가 점검 질문.
3. 원문/설명을 왜곡하지 말고, 시험·업무에 그대로 써도 될 만큼 정확하게 쓴다.
4. 사용자가 출력 형식을 요청하면 반영한다:
   - markdown: 기본 마크다운
   - pdf: PDF로 넘기기 좋게 페이지 구분(\`---\`)과 여백을 고려한 문단
   - image: 필기 카드 형태(큰 제목 + 짧은 블록)로 구성해 캡처하기 쉽게
5. 모든 본문은 한국어.`;

const MATH_SOLVE_INSTRUCTION = `너는 오류가 없는 수학 1타 강사이다. 문제 풀이는 단계별로 엄밀하게, 실수 없이 전개한다.

[출력]
- 마크다운
- 1) 문제 이해 2) 풀이 전략 3) 단계별 전개(식·이유) 4) 최종 답 5) 검산
- 기호·분수는 명확히. 애매하면 가정을 명시.
- 이미지 문제가 있으면 판독 후 풀이. 판독 불가는 명시.
- 한국어.`;

const EXAM_MAKER_INSTRUCTION = `너는 기출 범위에 맞춘 모의 시험지 출제위원이다.

사용자가 과목·범위·키워드를 주면 약 20문항의 모의 시험지를 만든다.

마크다운 출력:
# 모의 시험지
- 과목, 범위, 제한시간 제안
## 문항 (1~20)
각 문항: 번호, 배점, 문제, (객관식이면 보기)
## 정답 및 해설 (별도 섹션, 접어두기 쉬운 형태로)

범위 밖 문항 금지. 난이도 상·중·하 혼합. 한국어.`;
...
const VIDEO_SUMMARY_INSTRUCTION = `너는 영상 학습 요약 전문가이다. 사용자가 영상 URL을 주거나 영상/오디오/대본을 첨부하면 요약 정리한다.

[입력]
- URL 텍스트, 또는 첨부 파일(영상 대본·오디오·프레임 이미지) 모두 처리.

[출력 마크다운]
1. 한 줄 요약
2. 타임라인형 핵심 포인트 (시간 추정 가능하면 표시)
3. 상세 정리
4. 액션 아이템 / 복습 질문
판독 불가는 명시. 한국어.`;
...
export const TOOLS: ToolDef[] = [
  ...
  },
  {
    ...
    placeholder: "YouTube 등 영상 URL을 붙여넣거나, 파일 첨부 후 요청을 적어 주세요.",
    submitLabel: "영상 요약하기",
  ...
  },
  {
    ...
    title: "A4 노트 정리",
    description:
      "필기·설명을 A4 분량의 세밀한 노트로 정리합니다. 마크다운·PDF·이미지용 레이아웃을 요청할 수 있습니다.",
    icon: NotebookText,
    inputType: "text",
    outputType: "markdown",
    systemInstruction: NOTE_A4_INSTRUCTION,
    placeholder: "정리할 내용 + (선택) 출력 형식: markdown / pdf / image",
    submitLabel: "노트 만들기",
  ...
  },
  {
    ...
    placeholder: "문제 텍스트 또는 사진 첨부 후 풀이를 요청하세요.",
    submitLabel: "풀이하기",
  ...
  },
  {
    ...
    short: "시험지 제작",
    title: "모의 시험지 제작",
    description: "기출 과목·범위·키워드를 알려주면 약 20문항 모의 시험지와 해설을 생성합니다.",
    icon: ClipboardList,
    inputType: "text",
    outputType: "markdown",
    systemInstruction: EXAM_MAKER_INSTRUCTION,
    placeholder: "예) 고2 수학 미적분, 삼각함수·극한 범위, 난이도 중상",
    submitLabel: "시험지 만들기",

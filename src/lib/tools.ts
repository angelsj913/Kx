...
3) 흐름 예:
   ...
   - closing
4) 내용 밀도:
   - content bullets 4~7개 (한 줄에 정보 최대한, 군더더기 금지)
   - 표·다이어그램 슬라이드를 전체의 30% 이상 포함
   - 최소 table 1장 + diagram(process|cycle|cards) 1장 이상
   - 전문 용어 + 쉬운 비유를 같이
5) notes: 발표자가 그대로 읽을 수 있는 대본, 청중 질문 예상 1개 포함 가능
6) 모든 텍스트 한국어. 빈 slides/빈 table 금지.
7) JSON 외 문자 금지.`;
...
const VIDEO_SUMMARY_INSTRUCTION = `너는 영상·강의 요약 전문가이다. URL, 대본 텍스트, 오디오, 화면 캡처 이미지를 받아 학습용 요약을 만든다.

[입력 처리 우선순위]
1. 대본/메모 텍스트가 있으면 1순위 근거.
2. 오디오가 있으면 들은 내용 반영.
3. 이미지가 있으면 슬라이드/판서로 보완.
4. [영상 메타데이터] 블록(제목·채널·URL·비디오ID)이 있으면 제목·주제를 반영.
5. YouTube URL 직접 접근이 되면 내용 기반 요약, 안 되면 메타+첨부만으로 구조화하고 **한계를 상단에 명시**.

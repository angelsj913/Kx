...
export type { CheckResult, SecurityCheckOutcome, SecurityDomain, Severity } from "./checkIds";
export { DEFAULT_CHECK_IDS, domainForCheck, scoreFromOutcomes, severityCounts } from "./checkIds";

/** Repo root — turbopackIgnore prevents whole-project NFT tracing in API bundles. */
...
async function checkCorsSafe(): Promise<SecurityCheckOutcome> {
  ...
  try {
    const files = [...(await listRouteFiles()), "src/middleware.ts", "src/proxy.ts"];
    let anyAcao = false;
    let wildcard = false;
